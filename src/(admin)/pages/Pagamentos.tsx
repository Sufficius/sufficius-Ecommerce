...
export default function PagamentosPage() {
  ...
  // Estatísticas
  const estatisticas = {
    total: pagamento?.data?.reduce((acc: any, p: any) => acc + p.valor, 0),
    pago: pagamento?.data
      ?.filter((p: any) => p.status === "APROVADO")
      ?.reduce((acc: any, p: any) => acc + p.valor, 0),
    pendente: pagamento?.data
      ?.filter((p: any) => p.status === "PENDENTE")
      ?.reduce((acc: any, p: any) => acc + p.valor, 0),
    cancelado: pagamento?.data
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600">
                {pagamento.id} • {pagamento.pedidoId}
              </p>
            </div>
            <button
              onClick={onClose}
    ...
    <div className="py-8">
      ...
      <div className="bg-white rounded-xl shadow p-6 mb-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-sm text-gray-600">
            {metodoPagamento?.length} métodos utilizados
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y">
              {pagamentosPaginados?.map((pagamento: any) => {
